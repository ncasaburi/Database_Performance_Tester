...
class SubmenuPostgresRowUpdate():
    ...
    def __init__(self) -> None:
        ...
        #submenu items
        postgres_row_update_default = FunctionItem("Update default rows", self.postgres_row_update_fn, args=["default"]) 
        postgres_row_update_custom = FunctionItem("Update custom rows", self.postgres_row_update_fn, args=["custom"])
    ...
    def postgres_row_update_fn(self, type:str):
        ...
        try:
            ...
            if type == "default":
                ...
                postgres.run_query("UPDATE medical_records SET discharge_date = '"+datetime.now().strftime('%Y-%m-%d')+"' WHERE id_medical_record IN ( SELECT id_medical_record FROM medical_records ORDER BY id_medical_record DESC LIMIT "+str(rows_to_update)+")","Updating "+str(rows_to_update)+" relationships between doctors and medical records...")
