...
class SubmenuPostgresRowInsert():
        ...
        try:
            ...
            if type == "default":
                ...
                if rows_left > 0 and actual_doctor_rows == Config().default_postgres_lines_read and actual_patient_rows == Config().default_postgres_lines_read and actual_medicalrecord_rows == Config().default_postgres_lines_read and actual_doctormedicalrecord_rows == Config().default_postgres_lines_read:
                    ...
                    os.system('clear')
                    print("Doctors:")
                    self._load_sql_content("Doctors", requested_rows, default_insert_set, Config().default_postgres_lines_read, Config().default_postgres_last_file_read, postgres)
                    os.system('clear')
                    print("Patients:")
                    self._load_sql_content("Patients", requested_rows, default_insert_set, Config().default_postgres_lines_read, Config().default_postgres_last_file_read, postgres)
                    os.system('clear')
                    print("MedicalRecords:")
                    self._load_sql_content("MedicalRecords", requested_rows, default_insert_set, Config().default_postgres_lines_read, Config().default_postgres_last_file_read, postgres)
                    os.system('clear')
                    print("Doctors - MedicalRecords:")
                    (requested_rows, default_insert_set,Config().default_postgres_lines_read,Config().default_postgres_last_file_read) = self._load_sql_content("Doctor_MedicalRecords", requested_rows, default_insert_set, Config().default_postgres_lines_read, Config().default_postgres_last_file_read, postgres)
                else:
                    os.system('clear')
    ...
    def _load_sql_content(self, element:str, requested_rows:int, default_insert_set:int, default_postgres_lines_read, default_postgres_last_file_read, postgres) -> list:
