...
def mongo_document_batch_delete_fn() -> None:
    """This function allows the user to execute batch document deletes into a Mongo database"""

    try:
        mongo = MongoDB()

        print("Batch document deletes\n")
        ...
        if documents_left > 0:
            while(confirmation_loop):
                ...
                print("Batch row deletes     ("+str(documents_left)+" documents left)\n")

                total_requested_documents = int(input("  Total number of documents you want to delete: "))               
                documents_per_batch = int(input("  Number of documents you want to delete per batch: "))
                gap = float(input("  Gap between batch deletes (in seconds): "))

                if total_requested_documents > documents_left:
                    print("\nThe total number of documents to be deleted can't be greater than the number of documents left")
                    input("\n\nPress enter to continue...")
                    continue

                if total_requested_documents == 0:
                    print("\nThe total number of documents to be deleted must be greater than 0")
                    input("\n\nPress enter to continue...")
                    continue
            ...
            if total_requested_documents % documents_per_batch != 0:
                for i in range(int(sets)-1):
                    mongo_document_delete_fn("default",documents_per_batch,i+1,int(sets))
                    os.system('clear')
                    print("\n  Waiting "+str(int(gap))+" seconds...")
                    time.sleep(int(gap))
                mongo_document_delete_fn("default",last_set_documents)
            else:
        ...
        SingleLogger().logger.exception("MongoDB: Error while executing batch document delete", exc_info=True)